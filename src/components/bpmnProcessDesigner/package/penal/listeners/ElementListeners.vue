		...
		<el-table stripe :data="elementListenersList" size="small" border>
			...
			<el-table-column label="操作" width="100px">
				<template #default="scope">
					...
					<el-button size="small" link style="color: #ff4d4f" @click="removeListener(scope.$index)">移除</el-button>
				</template>
			</el-table-column>
		</el-table>
		<div class="element-drawer__button">
			<XButton type="primary" preIcon="ep:plus" title="添加监听器" @click="openListenerForm(null)" />
		</div>
			...
			<el-table stripe :data="fieldsListOfListener" size="small" max-height="240" border fit style="flex: none">
				...
				<el-table-column label="操作" width="130px">
					<template #default="scope">
						...
						<el-divider direction="vertical" />
						<el-button size="small" link style="color: #ff4d4f" @click="removeListenerField(scope.$index)">移除</el-button>
					</template>
				</el-table-column>
			</el-table>
...
const resetListenersList = () => {
	...
	bpmnElementListeners.value =
		bpmnElement.value.businessObject?.extensionElements?.values?.filter((ex) => ex.$type === `${prefix}:ExecutionListener`) ?? [];
