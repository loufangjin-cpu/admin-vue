		...
		<el-table stripe :data="elementListenersList" size="small" border>
			...
			<el-table-column label="操作" width="90px">
				<template #default="scope">
					...
					<el-button size="small" link style="color: #ff4d4f" @click="removeListener(scope.row, scope.$index)">移除</el-button>
				</template>
			</el-table-column>
		</el-table>
		<div class="element-drawer__button">
			<XButton size="small" type="primary" preIcon="ep:plus" title="添加监听器" @click="openListenerForm(null)" />
		</div>
			...
			<el-table stripe :data="fieldsListOfListener" size="small" max-height="240" border fit style="flex: none">
				...
				<el-table-column label="操作" width="100px">
					<template #default="scope">
						...
						<el-divider direction="vertical" />
						<el-button size="small" link style="color: #ff4d4f" @click="removeListenerField(scope.row, scope.$index)">移除</el-button>
					</template>
				</el-table-column>
			</el-table>
...
const resetListenersList = () => {
	...
	bpmnElementListeners.value =
		bpmnElement.value.businessObject?.extensionElements?.values.filter((ex) => ex.$type === `${prefix}:TaskListener`) ?? [];
...
const saveListenerConfig = async () => {
	...
	// 保存其他配置
	otherExtensionList.value = bpmnElement.value.businessObject?.extensionElements?.values?.filter((ex) => ex.$type !== `${prefix}:TaskListener`) ?? [];
	updateElementExtensions(bpmnElement.value, otherExtensionList.value.concat(bpmnElementListeners.value));
	// 4. 隐藏侧边栏
