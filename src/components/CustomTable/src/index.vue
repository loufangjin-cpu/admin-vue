			...
			<template v-if="table.firstColumn">
				...
				>
					<template #default="scope">
						<el-radio v-model="radioVal" :label="scope.$index + 1" @click.stop="radioChange($event, scope.row, scope.$index + 1)"></el-radio>
					</template>
				</el-table-column>
				<!-- 序列号 -->
				<el-table-column
				...
				>
					<template #default="scope">
						<span>
							{{ isShowPagination ? (table.currentPage - 1) * table.pageSize + scope.$index + 1 : scope.$index + 1 }}
						</span>
					</template>
				</el-table-column>
			</template>
						...
						<template #default="scope">
							...
							<template v-if="item.canEdit">
								<single-edit-cell
									...
									ref="editCell"
								>
									<slot v-if="item.configEdit && item.configEdit.editSlotName" :name="item.configEdit.editSlotName" :scope="scope" />
								</single-edit-cell>
							</template>
							<!-- 字典过滤 -->
...
// 抛出事件
const emits = defineEmits(['save', 'page-change', 'handleEvent', 'radioChange', 'rowSort']);
// 获取所有插槽
...
const checkIsShow = (scope, item) => {
	...
	return isShow && isNoshow && !scope.row[item.field] && (item.isField ? scope.row[item.isField] : true) && totalTxt && totalTxt1;
};
// 单个编辑事件
...
.t-table {
	...
	.radioStyle {
		:deep(.el-radio) {
			...
			&:focus:not(.is-focus, :active, .is-disabled) .el-radio__inner {
				box-shadow: none;
			}
		}
