	...
	<ContentWrap>
		...
		<el-descriptions title="订单状态" :column="1">
			<el-descriptions-item label="订单状态: ">
				...
				<dict-tag v-if="order.status !== ''" :type="DICT_TYPE.TRADE_ORDER_STATUS" :value="order.status" />
			</el-descriptions-item>
		...
		<el-descriptions title="商品信息">
			<el-descriptions-item labelClassName="no-colon">
				<el-row :gutter="20">
					<el-col :span="15">
						<el-table stripe :data="order.items" border>
		...
		<el-descriptions column="6">
			...
			<el-descriptions-item label="订单调价: "> ￥{{ parseFloat((order.adjustPrice / 100.0) as unknown as string).toFixed(2) }}</el-descriptions-item>
		...
		<div v-for="group in detailGroups" :key="group.title">
			<el-descriptions v-bind="group.groupProps" :title="group.title">
				...
				<el-descriptions-item v-if="group.key === 'orderLog'" labelClassName="no-colon">
					<el-timeline>
						<el-timeline-item v-for="activity in detailInfo[group.key]" :key="activity.timestamp" :timestamp="activity.timestamp">
							{{ activity.content }}
						</el-timeline-item>
					</el-timeline>
				</el-descriptions-item>
							...
							>
								...
								<template v-if="pkgChild.valueKey === 'goodsList' && pkgInfo[pkgChild.valueKey]">
									<div v-for="(goodInfo, goodInfoIdx) in pkgInfo[pkgChild.valueKey]" :key="`goodInfo_${goodInfoIdx}`" style="display: flex">
										<el-image style="width: 100px; height: 100px; flex: none" :src="goodInfo.imgUrl" />
